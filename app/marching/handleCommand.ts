...
// Shared context passed to command handlers so logic is centralized
...
const PREPARATORY_COMMANDS: Set<AtomicCommand> = new Set([
  'FORWARD',
  'HALF STEPS',
  'LEFT',
  'RIGHT',
...
function handlePreparatory(cmd: AtomicCommand, ctx: CommandContext) {
  const { setCurrentPreparatoryCommand, currentPreparatoryCommandRef, showPopupForBeats, pushPrep } = ctx;
...
async function executeMarch(ctx: CommandContext) {
  const {
  ...
  ctx.setCurrentExecutionCommand('MARCH');
  ctx.showPopupForBeats('MARCH');
  ctx.pushExec('MARCH');
  ...
  switch (currentPreparatoryCommandRef.current) {
    ...
    case 'COLUMN RIGHT': {
      ...
      (window as unknown as { marchingStep: () => void }).marchingStep = () => {
        ...
        ctx.setFlight({ ...f2, members: f2.members.map((c) => ({ ...c })) });
      };
      (window as unknown as { marchingStep: () => void }).marchingStep();
      break;
    }
...
async function executeFace(ctx: CommandContext) {
  const {
    flight,
    setFlight,
    currentPreparatoryCommandRef,
  ...
  ctx.setCurrentExecutionCommand('FACE');
  ctx.showPopupForBeats('FACE');
  ctx.pushExec('FACE');
  ...
  await doMarch(prepCmd);
}

async function executeHalt(ctx: CommandContext) {
  const { flight, setFlight, commandInProgressRef, CADENCES: CADENCE_MAP } = ctx;
  const awaitDelay = (ms: number) => new Promise((res) => setTimeout(res, ms));
  ctx.setCurrentExecutionCommand('HALT');
  ctx.showPopupForBeats('HALT');
  ctx.pushExec('HALT');
...
function executeFallIn(ctx: CommandContext) {
  const { setFallInMode, setFallInPreview, lastMousePosRef, setFallInDir } = ctx;
  ctx.setCurrentExecutionCommand('FALL-IN');
  ctx.showPopupForBeats('FALL-IN');
  ctx.pushExec('FALL-IN');
  setFallInMode(true);
  setFallInPreview(
...
function executeRotateFallIn(ctx: CommandContext) {
  const { setFallInDir } = ctx;
  ctx.setCurrentExecutionCommand('ROTATE FALL-IN');
  ctx.showPopupForBeats('ROTATE FALL-IN');
  ctx.pushExec('ROTATE FALL-IN');
  setFallInDir((d: Direction) => ((d + 90) % 360) as Direction);
}

const EXECUTION_HANDLERS: Record<AtomicCommand, (ctx: CommandContext) => Promise<void> | void> = {
  MARCH: executeMarch,
  FACE: executeFace,
  HALT: executeHalt,
  'FALL-IN': executeFallIn,
  'ROTATE FALL-IN': executeRotateFallIn,
};

export async function handleCommandLogic({ cmd, ...ctx }: { cmd: AtomicCommand } & CommandContext) {
  const {
    setCurrentCommand,
  ...
  if (PREPARATORY_COMMANDS.has(cmd)) {
    handlePreparatory(cmd, ctx);
    return;
  }

  const handler = EXECUTION_HANDLERS[cmd];
  if (handler) {
    await handler(ctx);
  }
}
